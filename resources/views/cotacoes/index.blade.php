            ...
            <form action="{{ route('cotacoes.index') }}" method="get" class="form-filter">
                <div class="row">
                    <div class="form-group col-md-5">
                        <label for="numero_cotacao">Número da Cotação</label>
                        <input type="text" name="numero_cotacao" id="numero_cotacao" class="form-control"
                            value="{{ request('numero_cotacao') }}" />
                    </div>
                    <div class="form-group col-md-5">
                    ...
                    @foreach ($cotacoes as $cotacao)
                        <tr>
                            ...
                            <td class="text-right text-nowrap">
                                <a href="{{ route('cotacoes.show', $cotacao->id) }}" class="btn btn-sm btn-info"
                                    title="Visualizar"><i class="fa fa-eye"></i></a>
                                <a href="{{ route('cotacoes.edit', $cotacao->id) }}" class="btn btn-sm btn-primary"
                                    title="Editar"><i class="fa fa-pencil"></i></a>
                                @if ($cotacoes->total() > 0)
                                    {!! Form::open([
                                        'id' => 'form_excluir_' . $cotacao->id,
                                        'method' => 'delete',
                                        'route' => ['cotacoes.destroy', $cotacao->id],
